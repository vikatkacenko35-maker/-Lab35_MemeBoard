...
function createCardHTML(meme) {
  ...
  const stats = "⭐".repeat(meme.rating);
  return `
    <div class ="meme-card">
...
async function LoadMemes() {
  try {
    ...
    loadingText.style.display = "none";
    namesGrid.innerHTML = `<p class="empty-text"> Ошибка: ${error.message}</p>`;
  }
}
async function handleAdd() {
  const title = inputTitle.ariaValueMax.trim();
  const category = inputCategory.ariaValueMax;
